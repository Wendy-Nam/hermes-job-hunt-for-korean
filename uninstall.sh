...
# install.sh가 남긴 $DATA/.kit-manifest.txt(코드 파일 목록: skills/plugins/scripts/bin)만 지운다.
...
[ -f "$MANIFEST" ] || { echo "❌ $MANIFEST 없음 — install.sh로 설치한 적이 없거나, 매니페스트 도입 이전 버전으로 설치됨(수동으로 skills/plugins/scripts/bin 확인 필요)"; exit 1; }

N=$(sort -u "$MANIFEST" | grep -c . || true)
echo "  매니페스트: 파일 ${N}개 (skills/plugins/scripts/bin 아래 코드만 — wiki·SOUL·search-profile·cron·config는 그대로 둠)"
...
if [ "$DRY" = "1" ]; then
  ...
  # 빈 디렉터리 정리 (skills/plugins/scripts/bin 안쪽만, 얕은 것부터 안전하게)
